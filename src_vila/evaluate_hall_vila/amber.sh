...
if [[ $MODE == 'a' ]]; then
    QFILE="eval_hall_vila/amber/data/query/query_all.json"

elif [[ $MODE == 'g' ]]; then
    QFILE="eval_hall_vila/amber/data/query/query_generative.json"

elif [[ $MODE == 'd' ]]; then
    QFILE="eval_hall_vila/amber/data/query/query_discriminative.json"

elif [[ $MODE == 'de' ]]; then
    QFILE="eval_hall_vila/amber/data/query/query_discriminative-existence.json"

elif [[ $MODE == 'da' ]]; then
    QFILE="eval_hall_vila/amber/data/query/query_discriminative-attribute.json"

elif [[ $MODE == 'dr' ]]; then
    QFILE="eval_hall_vila/amber/data/query/query_discriminative-relation.json"
...
if [[ $MODEL_BASE == 'none' ]]; then
    ...
    CUDA_VISIBLE_DEVICES=$GPU_NUM python -m eval_hall_vila.model_amber_loader \
    --model-path $MODEL \
    --question-file ${QFILE} \
