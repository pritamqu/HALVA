'''
adopted from: https://github.com/junyangwang0410/AMBER/blob/master/inference.py
'''
...
def get_args():
    ...
    # make full path
    args.word_association=os.path.join(args.anno_dir, args.word_association)
    args.safe_words=os.path.join(args.anno_dir, args.safe_words)
    args.annotation=os.path.join(args.anno_dir, args.annotation)
    args.metrics=os.path.join(args.anno_dir, args.metrics)
...
def main(args):
    ...
    if args.inference_data.endswith('.jsonl'):
        inference_data = [json.loads(q) for q in open(os.path.expanduser(args.inference_data), "r", encoding='utf-8')]
    elif args.inference_data.endswith('.json'):
        inference_data = json.load(open(os.path.expanduser(args.inference_data), "r", encoding='utf-8'))
    else:
