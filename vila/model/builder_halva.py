...
from vila.model.language_model.llava_llama import LlavaConfig
    ...
    **kwargs,
    ...
    if is_mm_model(model_path):
        ...
        if "lora" in model_name.lower() and model_base is not None:
            # written by PS for Lora-based finetuning

            # first load the base model
            print("Loading VILA from base model...")
            base_config=AutoConfig.from_pretrained(model_base)
            prepare_config_for_eval(base_config, kwargs)
            ...
            print("Loading additional VILA weights...")
            ...
            ## debug
            for wname in non_lora_trainables:
                _w1=wname
                print(wname)
                break

            for wname in model.state_dict().keys():
                if 'mm_projector' in wname:
                    _w2=wname
                    print(wname)
                    break

            assert _w1==_w2, "[Error] Check mm_projector names"
            model.load_state_dict(non_lora_trainables, strict=False)

            from peft import PeftModel
            print("Loading LoRA weights...")
            model.llm = PeftModel.from_pretrained(model.llm, model_path) # there are naming issue in directly loading the weights using this func
            adapter_weights=torch.load(os.path.join(model_path, 'adapter_model.bin'), map_location="cpu") # separately loaidng weights
            adapter_weights_new={}
            for k, v in adapter_weights.items():
                if 'default' not in k:
                    new_key=k.split('.')[:-1]+['default']+[k.split('.')[-1]]
                    new_key='.'.join(new_key)
                    # print(new_key)
                    adapter_weights_new[new_key]=v
                else:
                    adapter_weights_new[new_key]=v
            

            for wname in model.state_dict().keys():
                if 'lora' in wname.lower():
                    print('expected adapter weight name', wname)
                    break
            for wname in adapter_weights:
                print('current adapter weight name', wname)
                break
            for wname in adapter_weights_new:
                print('modified adapter weight name', wname)
                break

            msg=model.load_state_dict(adapter_weights_new, strict=False)
            assert len(msg.unexpected_keys)==0, "[Error] Loading adapter weights failed"
